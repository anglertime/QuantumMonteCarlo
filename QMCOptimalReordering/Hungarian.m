function [Matching,Cost] = Hungarian(B)
% [MATCHING,COST] = Hungarian_New(WEIGHTS)

% The Hungarian algorithm allows a "minimum matching" to be found. This can
% be interpreted as instances where there are multiple quotes for a group of activities
% and each activity must be done by a different person, to find the minimum
% cost to complete all of the activities.
%
...
% Reference: Alex Melin, 2006.
%% Ming. October 2011.

 % Initialize Variables
 Matching = zeros(size(B));
...
% increase the speed of the algorithm
  ...
  % Assemble Condensed Performance Matrix
    ...
    P_cond = zeros(P_size);
    P_cond(1:length(x_con),1:length(y_con)) = B(x_con,y_con);
    if isempty(P_cond)
      Cost = 0;
      return
    end
    ...
    % Ensure that a perfect matching exists
      ...
      % exists, especially for MxN matrix
...
% Ming 2013-11-19 Comment the cost here.
% Cost = sum(sum(B(Matching==1)));
