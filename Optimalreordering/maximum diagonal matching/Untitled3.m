...
% B = A;
% tic;
% [ cutoff, bisec_it ] = max_min_diagonal( B, 2*size(B, 1)+2, 1e-3, 1e-2 );
% % particle = dlmread('30000particle.txt');
% 
% [ B_opt, newParticle, Matching ] = max_weighted_match( A, 0.02);
% t = toc

% global constK Box datafile detRatio_dense sideLength max_dist max_Acut dim 
% 
% dim = 3;
% [ParticleConfig, OrbitalConfig, Box, sigma, constK, sideLength, max_dist, max_Acut] = read_configuration(1024);
% 
% totalSteps = 1;
...
hold on
plot(real(eig(A30000)),imag(eig(A30000)), '*b')
grid on
hold on
plot(real(eig(C)),imag(eig(C)), '*m')
hold on
% plot(real(eig(C_do)),imag(eig(C_do)), '*r')
% Steps = fopen('particle30K.txt', 'wt+');
% save_config(Steps, 1024, newParticle);
% fclose(Steps);