function [ B_opt, newParticle, Matching ] = max_weighted_match( B, cutoff, multiple, particle)
...
% C(C > replace) = does not change;
C((C <= replace)&(C >= cutoff)) = replace + 0.01 - C(C >= cutoff);
C(C < 0) = 5*multiple*replace;
C((C >= 0)&(C < cutoff)) = multiple*replace;
