function [ cutoff_m, B_opt, k ] = opt_mat_bisectflow( B, size_nrm, tol )
...
% while (k < max_it) && (cutoff_m > 1e-6) && (abs(start - terminal) < 6e-5 )
while (k < max_it)    
%     fprintf('\n\nBisection step %d:  \n[A, B] = [%2.6f, %2.6f], cutoff_m = %2.6f \n', k + 1, start, terminal, cutoff_m);
    ...
    if max_flow_value == n
%         if (cutoff_m > 1e-5) && (abs(start - terminal) < tol )
%             break;
%         end
        start = cutoff_m;
...
%     fprintf('[A, B] = [%2.6f, %2.6f], cutoff_m = %2.6f. max_flow is %d \n', start, terminal, cutoff_m, max_flow_value);
