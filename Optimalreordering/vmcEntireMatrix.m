global constK Box datafile detRatio_dense sideLength max_dist max_Acut dim 

dim = 2;
N = num^dim;
totalSteps = 5000;

ParticleConfig = Pconfig; OrbitalConfig = Oconfig; sigma = sideln*(0.5 - 0.1*dim); constK = 1; sideLength = sideln;
max_Acut = 1e+6; % 1/(max_Acut) is the smallest nonzero value of A, like 1/1e+6 = 1e-6.
max_dist = log(max_Acut/3.5449024699805363)/constK;  % This is the cutoff radius when calculate A and u.
% [ParticleConfig, OrbitalConfig, Box, sigma, constK, sideLength, max_dist, max_Acut] = read_configuration(N);
...
[ cutoff, ~ ] = max_min_diagonal( A, 2*N+2, 1e-3, 1e-4, 50);
psum = sum(diag(A));
...
%     error1(mcStep) = err1;
   ...
   if (detRatio*detRatio > randNum)
       ...
       %reorder
%        if (mod(acceptCount, floor(0.2*N)) == 0) || (min(diag(A)) < min(0.5*cutoff, 8e-5)) || (sum(diag(A)) < psum - 1)
%            temp1 = min(diag(A)); temp2 = min(0.5*cutoff, 5e-4); temp3 = sum(diag(A)) - psum + 1;
%            [ cutoff, ~] = max_min_diagonal( A, 884, 1e-3, 1e-4, 50 );
%            [ A, ParticleConfig, Matching ] = max_weighted_match( A, cutoff, 3, ParticleConfig );
%            fprintf(datafile1, sprintf('\n At step %d, reordered the matrix. cutoff value is %f. \n min_diagA_before = %f, min_diagA_af = %f, min(0.5*cutoff, 5e-4) = %f, temp3 = %f \n', mcStep, cutoff, temp1, min(diag(A)), temp2, temp3));
%            psum = sum(diag(A));
%            %            cond1(reord) = cond(A);
%            if reord <= N
%                Match(reord, :) = Matching;
%            end
%            reord = reord + 1;
%        end
   else
