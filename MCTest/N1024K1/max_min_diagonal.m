function [ cutoff, bisec_it ] = max_min_diagonal( B, size_BGraph, cutoff_tol, bi_tol  )
...
% cutoff_tol : tol we want to round the cutoff value. It should be 1e-2,1e-3, 1e-4, etc.
...
fprintf('\nBisection steps %d, cutoff value %2.12f, max flow value is %d \n\n', bisec_it, cutoff, max_flow_value);
%%
% matching = FlowMatrix( 2:2+n-1, n+2:n+2 + (n-1));
% [part, orb] = find( matching > cutoff );
% B_opt = B;
% for i = 1:n
