function [ B_re, newParticle, permu, cutoff, localSize] = LocalReorder( B, Particle)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
global max_Acut indicator mat_L
N = size(B, 1);
...
%% Method 1. Nearby rows.
[~, CenterP] = min(diag(B));
dropTol = 10*1/max_Acut;
Par = find(B(CenterP, :) >= dropTol);
% Method 2. Nearby columns.
...
%%
[ cutoff, ~] = max_min_diagonal( LocalB, 2*m1+2, 3e-3, 1e-3 );
...
for i = 1:m1
    if LocalMatching(i) == i
        ...
        B_re(Par(i), :) = temp;
        temp1 = mat_L(Par(LocalMatching(i)), :);
        mat_L(Par(LocalMatching(i)), :) = mat_L(Par(i), :);
        mat_L(Par(i), :) = temp1;
